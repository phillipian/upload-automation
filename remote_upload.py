...
def fetch_writer_id(writer_str):
    ...
    try:
        cmd = 'wp user create ' + writer_login + ' ' + writer_login +" --role='author' --display_name='"+writer+"' --porcelain"
...
for s in sections:
    ...
    for article_txt in article_txts: # loop through articles and upload them
        ...
        cmd = "wp post create " + article_txt + " --post_category="+ categories +" --post_status=draft --post_title='"+ headline +"' --porcelain --post_author="+ writer_id + ' ' + more_options.strip()
